...
- (void)clickOnButtonWith:(UIButton *)button
{
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(actionSheet:clickedButtonAtIndex:)] == YES) {
            [self.delegate actionSheet:self clickedButtonAtIndex:(NSInteger)button.tag];
        }
    }
