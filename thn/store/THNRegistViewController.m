...
@interface THNRegistViewController ()<UITextFieldDelegate, JYComHttpRequestDelegate>
{
...
@implementation THNRegistViewController
...
- (void)addMobileRegisterViewToView:(UIView *)view{
    ...
    mobilePasswordTextField.placeholder = @"密码";
...
- (IBAction)getMobileAuth{
    ...
    if (![self judgePhoneNumber:phone]) {
        [self alertWithInfo:@"请输入正确的手机号！"];
        return;
    }
...
- (IBAction)mobileRegister{
    ...
    */
    ...
    [_request clearDelegatesAndCancel];
    _request.delegate = self;
    [_request postInfoWithParas:paras andUrl:[NSString stringWithFormat:@"%@%@", kTHNApiBaseUrl, kTHNApiRegister]];
    [self showHUDWithMessage:@"正在注册"];
}
