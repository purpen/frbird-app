...
- (void)requestUserInfo
{
    ...
    request = [[SinaWeiboRequest requestWithURL:kSinaWeiboUserInfo
                                     httpMethod:@"GET"
                                         params:params
                                       delegate:self] retain];
...
{
    if (_request == request && ![_request.url isEqualToString:kSinaWeiboUserInfo]){
        JYLog(@"access token result = %@", result);
        ...
        self.usericon = [result objectForKey:@"profile_image_url"];
        [request release], request = nil;
