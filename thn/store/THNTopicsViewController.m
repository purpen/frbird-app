...
@interface THNTopicsViewController ()<JYComHttpRequestDelegate>
...
@implementation THNTopicsViewController
{
    BOOL                    _nibsRegistered;
    THNTopicCategory        *_cateData;
    THNTopicCategory        *_mainCateData;
...
- (void)viewDidLoad {
    ...
    self.title = _cateData.cateTitle;
    _nibsRegistered = NO;
    _currentPage = 1;
    ...
    [rightButton setImage:[UIImage imageNamed:@"add contact"] forState:UIControlStateNormal];
...
- (void)requestForPage:(int)page
{
    ...
    NSMutableDictionary *listPara = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                     [NSString stringWithFormat:@"%ld",_cateData.cateID], @"category_id",
    ...
    [_contentRequest getInfoWithParas:listPara andUrl:[NSString stringWithFormat:@"%@%@", kTHNApiBaseUrl, kTHNApiPartyList]];
}
...
{
    THNAddTopicViewController *addTopic = [[THNAddTopicViewController alloc] initWithMainCate:_mainCateData];
